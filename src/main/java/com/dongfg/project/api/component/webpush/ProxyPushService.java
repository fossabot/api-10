 ...
 * @date 2018/1/2
 */
class ProxyPushService extends PushService {
    ...
    public Future<HttpResponse> sendAsync(Notification notification) throws GeneralSecurityException, IOException, JoseException {
        ...
        if (vapidEnabled() && !notification.isGcm()) {
            ...
            if (headers.containsKey(CRYPTO_KEY)) {
                headers.put(CRYPTO_KEY, headers.get(CRYPTO_KEY) + ";p256ecdsa=" + base64url.omitPadding().encode(pk));
            } else {
                headers.put(CRYPTO_KEY, "p256ecdsa=" + base64url.encode(pk));
            }
        }
